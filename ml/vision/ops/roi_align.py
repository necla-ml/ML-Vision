...
def _roi_align(input, rois, spatial_scale_height, spatial_scale_width, pooled_height, pooled_width, sampling_ratio, aligned):
    ...
    roi_start_w = rois[:, 1] * spatial_scale_width - offset  # [K]
    roi_start_h = rois[:, 2] * spatial_scale_height - offset  # [K]
    roi_end_w = rois[:, 3] * spatial_scale_width - offset  # [K]
    roi_end_h = rois[:, 4] * spatial_scale_height - offset  # [K]
...
def roi_align(
    ...
    spatial_scale: Union[float, tuple] = 1.0,
    ...
    Args:
        ...
        spatial_scale (float or Tuple[float, float]): a scaling factor that maps the box coordinates to
    ...
    spatial_scale = _pair(spatial_scale)
    if not isinstance(rois, torch.Tensor):
        rois = convert_boxes_to_roi_format(rois)
    if not torch.jit.is_scripting():
        if not _has_ops() or (torch.are_deterministic_algorithms_enabled() and input.is_cuda):
            return _roi_align(input, rois, spatial_scale[0], spatial_scale[1], output_size[0], output_size[1], sampling_ratio, aligned)
    _assert_has_ops()
    return torch.ops.mlvision.roi_align(
        input, rois, spatial_scale[0], spatial_scale[1], output_size[0], output_size[1], sampling_ratio, aligned
    )
...
class RoIAlign(nn.Module):
    ...
    def __init__(
        self,
        output_size: BroadcastingList2[int],
        spatial_scale: Union[float, tuple],
        sampling_ratio: int,
        aligned: bool = False,
    ):
        super().__init__()
