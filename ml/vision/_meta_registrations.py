...
def get_meta_lib():
    return torch.library.Library("mlvision", "IMPL", "Meta")


def register_meta(op_name, overload_name="default"):
    def wrapper(fn):
        if _has_ops():
            get_meta_lib().impl(getattr(getattr(torch.ops.mlvision, op_name), overload_name), fn)
...
def meta_roi_align(input, rois, spatial_scale_x, spatial_scale_y, pooled_height, pooled_width, sampling_ratio, aligned):
...
def meta_roi_align_backward(
    grad, rois, spatial_scale_x, spatial_scale_y, pooled_height, pooled_width, batch_size, channels, height, width, sampling_ratio, aligned
):
    torch._check(
